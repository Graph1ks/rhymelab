...
const I18N = {
  en: {
    title:'Rhyme Bureau Local',eyebrow:'UNIFIED RHYME WRITER',headline:'Find rhymes across words, phrases and names.',
    intro:'Search German, English or both. Rhyme Bureau combines every locally available channel and still lets you isolate words, Phrase/Mosaic matches or Entities.',
    ...
    partOfSpeech:'Part of speech',lemma:'Lemma',usageRank:'Usage rank',corpusCount:'Corpus count',lexicalTags:'Lexical tags',category:'Category',categories:'Categories',popularity:'Popularity',phraseTypes:'Phrase types',wordBoundaries:'Word boundaries',entityCategory:'Entity category',allEntities:'All entities',sources:'Sources',sourcesTitle:'Rhyme Bureau Sources',sourcesIntro:'Active source families used by the current Rhyme Bureau data and runtime pipelines.',close:'Close',more:'More',loaded:'loaded',searchPool:'candidates in search pool',
  ...
  de: {
    title:'Rhyme Bureau Lokal',eyebrow:'VEREINHEITLICHTER REIM-WRITER',headline:'Reime für Wörter, Phrasen und Namen finden.',
    intro:'Deutsch, Englisch oder beides durchsuchen. Rhyme Bureau vereint alle lokal verfügbaren Kanäle und lässt Wörter, Phrase/Mosaic-Treffer und Entitäten trotzdem getrennt auswählen.',
    ...
    partOfSpeech:'Wortart',lemma:'Lemma',usageRank:'Gebrauchsrang',corpusCount:'Korpus-Treffer',lexicalTags:'Lexikalische Tags',category:'Kategorie',categories:'Kategorien',popularity:'Popularität',phraseTypes:'Phrasentypen',wordBoundaries:'Wortgrenzen',entityCategory:'Entity-Kategorie',allEntities:'Alle Entities',sources:'Sources',sourcesTitle:'Rhyme Bureau Sources',sourcesIntro:'Aktive Quellenfamilien der aktuellen Rhyme-Bureau-Daten- und Runtime-Pipelines.',close:'Schließen',more:'Mehr',loaded:'geladen',searchPool:'Kandidaten im Suchpool',
...
const SOURCE_CATALOG=Object.freeze([
  ...
  {name:'Rhyme Bureau curated modern lexicon',detail:{en:'Reviewed supplemental modern vocabulary for product gaps.',de:'Geprüfte ergänzende moderne Vokabeln für Produktlücken.'}},
...
const savedBasis=localStorage.getItem('rhymelab.searchBasis');
...
function searchStickyTop(){
  ...
  const height=Number(rect?.height||topbar?.offsetHeight||0);
  return Math.max(8,Math.ceil(height)+8);
}
...
function syncGeneratedOptinControl(){
  ...
  const line=$('#generatedFilter');
  const onlyLine=$('#generatedOnlyFilter');
  if(!input||!onlyInput)return;
  const available=state.generatedCapability?.available===true;
  if(!available){state.generatedOptIn=false;state.generatedOnly=false;}
  if(state.generatedOnly&&available)state.generatedOptIn=true;
  input.disabled=!available;
  onlyInput.disabled=!available;
  input.checked=Boolean(state.generatedOptIn&&available);
  onlyInput.checked=Boolean(state.generatedOnly&&state.generatedOptIn&&available);
  line?.classList.toggle('unavailable',!available);
  onlyLine?.classList.toggle('unavailable',!available);
  if(line)line.title=available?'':t('generatedUnavailable');
  if(onlyLine)onlyLine.title=available?'':t('generatedUnavailable');
...
function renderDatasetStats(){
  ...
  if(!stats){
    node.innerHTML=`<div class="status">${esc(t('statsUnavailable'))}</div>`;
    return;
  }
  const rows=[
    ['deWords',t('statsDeWords')],
    ['enWords',t('statsEnWords')],
    ['phrases',t('statsPhrases')],
    ['entities',t('statsEntities')],
  ];
  const rowHtml=rows.map(([key,label])=>{
    const item=stats.categories?.[key]||{};
    return `<tr><th scope="row">${esc(label)}</th><td>${number(item.core||0)}</td><td>${number(item.generated||0)}</td><td><strong>${number(item.total||0)}</strong></td></tr>`;
  }).join('');
  const totals=stats.totals||{};
  node.innerHTML=`<div class="stats-summary"><span>${esc(t('statsUnit'))}</span><strong>${number(totals.total||0)}</strong></div><div class="stats-table-wrap"><table class="stats-table"><thead><tr><th></th><th>${esc(t('coreData'))}</th><th>${esc(t('generatedShort'))}</th><th>${esc(t('total'))}</th></tr></thead><tbody>${rowHtml}</tbody><tfoot><tr><th scope="row">${esc(t('grandTotal'))}</th><td>${number(totals.core||0)}</td><td>${number(totals.generated||0)}</td><td><strong>${number(totals.total||0)}</strong></td></tr></tfoot></table></div>`;
}
function renderRuntimeTiming(){
  const timing=state.runtimeTiming;
  const current=$('#runtimeTimingCurrent');
  const average=$('#runtimeTimingAverage');
  const samples=$('#runtimeTimingSamples');
  const card=$('#runtimeTiming');
  if(!current||!average||!samples||!card)return;
  const searchMs=Number(timing?.searchMs);
  const averageMs=Number(timing?.averageLast100Ms);
  current.textContent=Number.isFinite(searchMs)?number(Math.round(searchMs)):'—';
  average.textContent=Number.isFinite(averageMs)?number(Math.round(averageMs)):'—';
  const sampleCount=Math.max(0,Number(timing?.sampleCount||0));
  const windowSize=Math.max(1,Number(timing?.windowSize||100));
  samples.textContent=`${number(sampleCount)} / ${number(windowSize)}`;
  card.classList.toggle('has-samples',sampleCount>0);
}

async function loadDatasetStats(){
  if(state.datasetStats){renderDatasetStats();return state.datasetStats;}
  const node=$('#statsContent');
  if(node)node.innerHTML=`<div class="status">${esc(t('statsLoading'))}</div>`;
  try{
    const response=await fetch('/api/dataset-stats');
    if(!response.ok)throw new Error('dataset_stats_unavailable');
    state.datasetStats=await response.json();
    renderDatasetStats();
    return state.datasetStats;
  }catch{
    state.datasetStats=null;
    if(node)node.innerHTML=`<div class="status error">${esc(t('statsUnavailable'))}</div>`;
    return null;
  }
}
...
async function loadCapabilities(){try{const response=await fetch('/api/health');if(!response.ok)return;const health=await response.json();state.capabilities=health.unified_writer||null;state.generatedCapability=health.generated_optin||{available:false,reason:'generated_optin_unavailable'};state.canonicalPronunciationRevision=health.query_pronunciation_revision||null;state.generatedPronunciationRevision=state.generatedCapability?.query_pronunciation_revision||null;syncGeneratedOptinControl();syncCapabilityControls();applyLanguage();renderCapabilityNotice();}catch{}}
...
async function lookupSourceBackedWord(surface,language){
  ...
  try{
    const response=await fetch(`/api/word/${encodeURIComponent(surface)}?language=${encodeURIComponent(language)}&generated=${state.generatedOptIn?'1':'0'}`);
...
function reportUiInitializationFailure(error){
  ...
  banner.textContent='Rhyme Bureau UI controls failed to initialize. Reload after updating the local app; check the browser console if the problem persists.';
  const shell=$('.shell');
