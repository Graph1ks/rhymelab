...
const I18N = {
  en: {
    ...
    modern:'Modern lexicon',historical:'Historical',syllableShort:'syll.',modernEntity:'Modern',historicalEntity:'Historical',inspectWord:'Inspect',
  ...
  de: {
    ...
    modern:'Modernes Lexikon',historical:'Historisch',syllableShort:'Silb.',modernEntity:'Modern',historicalEntity:'Historisch',inspectWord:'Details zu',
...
function populateEntityCategories(){const select=$('#entityCategory');if(!select)return;const current=select.value||'all',categories=[...(state.capabilities?.entities?.categories||[])].sort((a,b)=>String(a).localeCompare(String(b),'en'));const groups=new Map();for(const category of categories){const [family='entity']=String(category).split('.');if(!groups.has(family))groups.set(family,[]);groups.get(family).push(category);}select.innerHTML=`<option value="all">${esc(t('allEntities'))}</option>`+[...groups.entries()].sort(([a],[b])=>a.localeCompare(b,'en')).map(([family,items])=>`<optgroup label="${esc(humanize(family))}">${items.map((category)=>`<option value="${esc(category)}">${esc(humanize(category))}</option>`).join('')}</optgroup>`).join('');select.value=categories.includes(current)?current:'all';}
...
function renderWordPanel(query,rhymeResult=null,displayType=null){
  ...
  const layer=query.lexiconLayer==='modern'?t('modern'):t('dictionary');
  ...
  $('#wordPanel').innerHTML=`<div class="word-heading"><div><h2>${esc(query.surface)}</h2><div class="ipa">/${esc(query.preferredIpa||preferred?.ipa||'')}/</div></div><div class="layer-badges"><span class="layer-badge ${query.lexiconLayer==='modern'?'modern':''}">${esc(layer)}</span>${query.historical?`<span class="layer-badge historical">${t('historical')}</span>`:''}</div></div><div class="meta-grid"><div class="meta-item"><span class="meta-label">${t('syllableCount')}</span><span class="meta-value">${esc(query.syllableCount??t('unknown'))}</span></div><div class="meta-item"><span class="meta-label">${t('primaryStress')}</span><span class="meta-value">${esc(query.primaryStressSyllable??t('unknown'))}</span></div><div class="meta-item"><span class="meta-label">${t('usageRank')}</span><span class="meta-value">${query.usageRank?`#${number(query.usageRank)}`:t('unranked')}</span></div><div class="meta-item"><span class="meta-label">${t('corpusCount')}</span><span class="meta-value">${query.usageCount!=null?number(query.usageCount):'—'}</span></div><div class="meta-item"><span class="meta-label">${t('partOfSpeech')}</span><span class="meta-value">${esc(grammaticalLabel)}</span></div><div class="meta-item"><span class="meta-label">${t('lemma')}</span><span class="meta-value">${esc(query.lemma||'—')}</span></div></div>${rhymeAnalysisHtml(rhymeResult,displayType)}${tagHtml}<div class="pronunciation-list"><h3>${t('variants')}</h3>${pronunciationHtml||'—'}</div>`;
}
...
function renderEntityPanel(result,displayType=null){
  const categories=(result.entityCategories||[]).map((entry)=>entry.category).filter(Boolean);
  const popularity=result.popularityPercentile!=null?Math.round(Number(result.popularityPercentile)*100):null;
  $('#wordPanel').innerHTML=`<div class="word-heading"><div><h2>${esc(result.surface||result.word)}</h2><div class="ipa">/${esc(result.ipa||'')}/</div></div><div class="layer-badges"><span class="layer-badge modern">${t('entity')}</span><span class="layer-badge">${String(result.language||'').toUpperCase()}</span></div></div><div class="meta-grid"><div class="meta-item"><span class="meta-label">${t('syllableCount')}</span><span class="meta-value">${esc(result.syllableCount??'—')}</span></div><div class="meta-item"><span class="meta-label">${t('category')}</span><span class="meta-value">${esc(result.primaryCategory?humanize(result.primaryCategory):'—')}</span></div><div class="meta-item"><span class="meta-label">${t('popularity')}</span><span class="meta-value">${popularity==null?'—':`${popularity}% · ${esc(result.popularityTier||'—')}`}</span></div><div class="meta-item"><span class="meta-label">QID</span><span class="meta-value">${esc(result.entityQid||'—')}</span></div><div class="meta-item meta-wide"><span class="meta-label">${t('categories')}</span><span class="meta-value">${esc(categories.length?categories.map(humanize).join(', '):'—')}</span></div></div>${rhymeAnalysisHtml(result,displayType)}`;
}
function resultRow(row,displayType){const isPhrase=row.resultKind==='phrase',isEntity=row.resultKind==='entity',usage=isEntity?(row.popularityTier||'—'):isPhrase?(row.usageCount?number(row.usageCount):'—'):(row.usageRank?`#${number(row.usageRank)}`:'—'),usageSub=isEntity?(row.primaryCategory?humanize(row.primaryCategory):t('entity')):isPhrase?'Leipzig':(row.usageCount!=null?number(row.usageCount):t('unranked')),score=displayScore(row,displayType),key=`${row.resultKind||'word'}:${row.resultId||row.windowId||row.normalized||row.word}`,kindClass=isPhrase?'phrase':isEntity?'entity':'word',kindLabel=isPhrase?t('phrase'):isEntity?`${String(row.language||'').toUpperCase()} · ${t('entity')}`:`${String(row.language||'de').toUpperCase()} · ${t('word')}`;return`<div class="result-row ${isPhrase?'phrase-row':isEntity?'entity-row':''}" data-result-key="${esc(key)}" data-display-type="${esc(displayType)}" tabindex="0" aria-label="${esc(`${t('inspectWord')} ${row.word}`)}"><div class="result-word"><div class="result-title-line"><strong>${esc(row.word)}</strong><span class="kind-chip ${kindClass}">${kindLabel}</span></div><div class="sub"><code>${isPhrase&&row.ipaKind==='matched_mosaic_span'?'span: ':''}/${esc(row.ipa||'')}/</code>${row.historical?`<span class="historical-text">${t('historicalEntity')}</span>`:''}</div></div><div class="type-col"><span class="badge ${esc(displayType)}">${esc(typeLabel(displayType))}</span></div><div class="syllable-col metric">${esc(row.syllableCount??'—')}<span class="muted">${t('syllableShort')}</span></div><div class="usage-col metric">${esc(usage)}<span class="muted">${esc(usageSub)}</span></div><div class="score">${Math.round(score*100)}%</div></div>`;}
