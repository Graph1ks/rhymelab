...
export function searchEntityRhymes(db, query, options = {}) {
  ...
  const legacyLookup=servingV1?null:db.prepare(`
    ...
    LIMIT ?
  `);
  const servingLookupIds=servingV1?db.prepare(`
    SELECT
    ...
    ORDER BY
      ...
      ra.qid,ra.name_id,ra.product_pronunciation_id
    LIMIT ?
  `):null;

  const hydrateServingEntityPronunciations=(ids)=>{
    const byId=new Map();
    ...
    for(let offset=0;offset<unique.length;offset+=400){
      ...
      const rows=db.prepare(`
        SELECT
          ...
          ep.source_kind,ep.source_record,ep.generated,ep.model_id,
          ep.confidence,ep.review_state,
          n.entity_id,n.surface,n.normalized,n.language,n.name_kind,
          n.preferred AS name_preferred,
          e.qid,e.primary_category,e.popularity_score,
          e.popularity_percentile,e.popularity_tier,
        ...
        WHERE ep.product_pronunciation_id IN (${marks})
      `).all(...batch);
      for(const row of rows)byId.set(Number(row.pronunciation_id),row);
    }
    return byId;
  };
  ...
  for(const anchor of anchors){
    ...
    for(const row of rows){
      ...
      const current=byPronunciation.get(pronunciationId);
      if(!current){
        byPronunciation.set(pronunciationId,{
          ...row,
  ...
  if(servingV1&&category==='all'&&byPronunciation.size){
    timed('anchor_hydration_ms',()=>{
      ...
      for(const [pronunciationId,current] of [...byPronunciation]){
        const row=hydrated.get(Number(pronunciationId));
        if(!row
          ||(queryNormalized
            &&profile.normalizeSurface(row.surface)===queryNormalized)){
          byPronunciation.delete(pronunciationId);
          continue;
        }
        byPronunciation.set(pronunciationId,{
          ...row,
          retrievalChannels:current.retrievalChannels,
        });
      }
    });
  }
