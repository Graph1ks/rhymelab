...
export function getPhraseBrowserStats(db){
 ...
 return{schema:'rhymelab-phrase-browser-stats-v1',phrases:Number(basic.phrases),modernEligible:Number(basic.modern_eligible),historicalOnly:Number(basic.historical_only),leipzig:{matchedPhrases:Number(usage.matched_phrases),rows:Number(usage.rows)},register:register?{matchedPhrases:Number(register.matched_phrases),rows:Number(register.rows),occurrences:Number(register.occurrences||0)}:null,rueg:rueg?{units:Number(rueg.units),diplTokens:Number(rueg.dipl_tokens||0),normTokens:Number(rueg.norm_tokens||0)}:null,sources};
}
...
export function searchPhrases(db,{q='',type='all',historical=false,evidence='all',limit=50}={}){
 ...
 const hasRegister=tableExists(db,'phrase_register_evidence'),hasRueg=tableExists(db,'phrase_register_occurrence');
 ...
 else if((evidence==='register'&&!hasRegister)||(evidence==='rueg'&&!hasRueg))return[];
...
export function getPhraseDetail(db,id){
 ...
 const detail=rowPhrase({...base,leipzig_corpus_count:0,leipzig_occurrences:0,leipzig_per_million:0,register_sources:0,register_occurrences:0,rueg_occurrences:0});
