...
document.addEventListener('click',e=>{const b=e.target.closest('button');if(!b)return;if(b.dataset.insert)insertWord(b.dataset.insert);if(b.dataset.save)toggleSave(b.dataset.save);if(b.dataset.query){query=b.dataset.query;void refreshWriterResults()}if(b.dataset.retryWriter){void refreshWriterResults()}if(b.dataset.detail)openDetail(b.dataset.detail,b.dataset.detailId||'');if(b.dataset.song){revision();state.active=b.dataset.song;activeLine=0;selection={line:0,start:0,end:0};undo=[];persist();renderEditor();navigate('studio');setMode('write')}if(b.dataset.trash){const s=state.songs.find(s=>s.id===b.dataset.trash);if(s.id===state.active){notify('Öffne zuerst einen anderen Text, um diesen abzulegen.');return}s.deleted=true;persist();renderLibrary();renderProjects();notify('Im Papierkorb · wiederherstellbar')}if(b.dataset.restore){state.songs.find(s=>s.id===b.dataset.restore).deleted=false;persist();renderLibrary(true);renderProjects()}});
...
function openDetail(word,id=''){
  ...
  if(!row)return;
  selectedResult=row.word;selectedResultId=row.id;selectedDetail=null;selectedDetailStatus='idle';selectedDetailError='';
...
function closeDetail(){detailClient.cancel();detailRequest++;selectedResult='';selectedResultId='';selectedDetail=null;selectedDetailStatus='idle';selectedDetailError='';$('#detailDock').classList.add('hidden');$('#resultsScroll').focus()}
