...
import {$,$,esc,icon,clamp} from './studio-core.mjs';
...
function data(){return getDemoSearchRows({query,scope,relation,resultLanguage:resultLang,queryBasis:basis})}
...
let density=normalizeDensity(state.density),syllableMode='all',followSelection=true,selectedResult='',dockTab='',resultSignature='',selectionProof=null;
...
function setFontSize(n){state.fontSize=clamp(n,16,28);document.documentElement.style.setProperty('--editor',state.fontSize+'px');$('#fontSizeLive').textContent=state.fontSize;$$('#lyrics textarea').forEach(resizeArea);persist()}
