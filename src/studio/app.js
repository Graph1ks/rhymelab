...
document.addEventListener('keydown',e=>{if((e.ctrlKey||e.metaKey)&&e.key.toLowerCase()==='k'){e.preventDefault();showCommands()}if(e.altKey&&e.key==='1'){e.preventDefault();navigate('studio')}if(e.altKey&&e.key==='2'){e.preventDefault();navigate('search')}if(e.altKey&&e.key.toLowerCase()==='f'){e.preventDefault();toggleFocus()}});window.addEventListener('resize',()=>queryAll('#lyrics textarea').forEach(resizeArea));document.addEventListener('visibilitychange',()=>{if(document.hidden){stopPlay();revision();persist()}});window.addEventListener('pagehide',()=>{revision();persist()});document.body.dataset.controls='bound';}
...
function resetInline(){scope='all';relation='all';sort='recommended';basis='de';resultLang='both';syllableMode='all';pageSize=12;queryAll('[data-scope]').forEach(b=>{b.classList.toggle('active',b.dataset.scope==='all');b.setAttribute('aria-pressed',b.dataset.scope==='all')});renderResults()}
