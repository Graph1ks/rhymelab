...
Current owner-only bulk cleanup: `docs/PRONUNCIATION_BACKFILL_V2.md` is authoritative. Before adapting source paths or starting the corrected full-data collection, run `npm run data:inventory` and use `data/local/local-data-inventory-v1-report.json` as the evidence for the owner's actual heterogeneous `data/` layout and file structures. V1 is superseded because it incorrectly scanned only already materialized runtime/accepted DBs. V2 builds the workset from source/stage → accepted diffs: German pre-publish core plus raw listed-form source, original English Kaikki Writer candidates, the pronunciation-independent Phrase catalog/token inventory, and the retained pronunciation-independent Entity name catalog. The corrected work DB is `data/local/pronunciation-backfill-v2.sqlite`. eSpeak-NG runs first; only analyzer-rejected rows continue through `client-total-query-pronunciation-v2`; generated evidence is classified A/B/C/D/U. Canonical runtime DBs are not mutated, and the owner has **not yet run** the corrected full-data workflow. Rerunning resumes committed collection/generation state; changed inputs fail closed.
