...
Current owner-only bulk cleanup: `docs/PRONUNCIATION_BACKFILL_V1.md` defines a separate resumable local staging workflow. It collects represented pronunciation gaps across DE Writer invariants, English forms, Phrase/Mosaic unresolved tokens + phrase surfaces, and DE/EN Entity names into `data/local/pronunciation-backfill-v1.sqlite`. The generator chain is existing eSpeak-NG adapter first, then `client-total-query-pronunciation-v2` only for eSpeak analyzer-rejections, with A/B/C/D/U quality tiers and continuous progress/rate/ETA output. The staging workflow does not mutate canonical runtime DBs and has **not yet been owner-run**. Rerunning the same command resumes from SQLite checkpoints; changed input DB revisions fail closed and require `--reset`.
