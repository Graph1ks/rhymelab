...
The current implementation gate is **Phase 12C proper-name token G2P benchmark v2**. Source expansion remains accepted at 710,561 / 1,415,550 EN Entity names ready (50.20%), with 704,989 unresolved. The first 600-case G2P benchmark preparation is rejected as a control-set design defect, not a model result: 599/600 cases were single-word whole Entity surfaces, 592/600 had CMUdict references, and only 578 normalized surfaces were distinct. No G2P was executed. The corrected v2 benchmark operates on unique Entity-name tokens and uses explicit en-US Kaikki/Wiktionary proper-name IPA as gold; CMUdict is excluded as benchmark gold. After merge, do not add another intermediate upload gate: one owner command `npm run entity:g2p:benchmark:mfa` rebuilds v2, enforces the v2 structural invariants, runs the pinned MFA model, and evaluates it. Primary model target is MFA public release `english_us_arpa` v2.0.0a because ARPA output feeds the accepted English analyzer directly. Verify the stable model family (Pynini + complete 69-phone ARPA inventory) and record the actual `mfa model inspect` internal version/fingerprint; do not require the inspect build string to equal the public release label. The first MFA owner run is rejected as a runner defect: only 13/600 predictions mapped because display-case Proper Names were sent to an MFA model whose grapheme inventory is lowercase a-z + apostrophe; the 13-case quality metrics are not model evidence. The fixed runner explicitly normalizes model inputs, records diacritic folding/ineligible graphemes, and hard-fails below 95% eligible prediction coverage. Run the owner command only inside Miniforge Prompt with `(rhymelab-mfa)` active. Do not promote generated Entity pronunciations before the full-coverage v2/MFA evidence is reviewed.
