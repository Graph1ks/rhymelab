...
The current implementation gate is **Phase 12C MFA confidence calibration**. Source expansion remains accepted at 710,561 / 1,415,550 EN Entity names ready (50.20%), with 704,989 unresolved. Proper-name benchmark v2 is the accepted control: 600 unique Entity-name tokens with explicit en-US Kaikki/Wiktionary proper-name IPA. The corrected full MFA owner run is valid: 599/600 model-input eligible, 100% eligible prediction coverage, 597 evaluated, exact phones 70.35%, exact stressed rhyme tail 71.19%, syllable count 95.98%, stress pattern 80.57%, primary stress 94.14%, mean rhyme score 0.913040, evaluation fingerprint `b6c3943f90a90a2938bcd3ce74ccecc95eedc67712fe751a6dda0266b2c91b5a`. Do not promote MFA unconditionally: 71.19% exact-tail is insufficient for blind rhyme-anchor generation. Before any second-model benchmark, calibrate MFA's own exported Pynini path score on the same benchmark. The runner now uses `--export_scores`; the evaluator reports retained quality at 10/25/50/75/90/100% and the maximum retained subset that reaches 90% / 85% exact-tail. Owner command must run inside Miniforge Prompt with `(rhymelab-mfa)` active: `npm run entity:g2p:benchmark:mfa`. No generated pronunciation is runtime-promoted by this gate.
