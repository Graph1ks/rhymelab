...
Current work is **12A3 retained Entity catalog + conservative DE pronunciation/phonetic runtime + source-coverage diagnostics + RhymePad Entity channel**. Read `docs/ENTITY_PRONUNCIATION_RUNTIME_V1.md`. The first owner full-data pronunciation build completed at 90,224 / 716,940 DE names (12.58%), 90,224 analyses, 569,995 anchors, fingerprint `38199d5b872c3fd2a20839490005f43d76ac6baaecfe657b1026d3d94efd66b3`. Do not refetch QLever or restage Wikidata/QRank. Do not invent IPA: preserve eligible source-backed pronunciations and otherwise compose only exact Writer-v5 token pronunciations when every token resolves. Unresolved names remain out of phonetic retrieval.

Use `npm run entity:pronunciation:owner` for the normal owner diagnostic path. It preserves a complete local runtime, verifies/fetches the pinned CMUdict probe, and produces category/tier/preferred-name coverage plus priority unresolved entities. CMUdict is North American English source evidence and remains probe-only in 12A3: never relabel it as `de-DE`, never pass it into `de-ipa-v2`, and never enable it in runtime retrieval until an English phonology/runtime contract is accepted.
