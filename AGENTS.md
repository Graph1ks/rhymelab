...
The current implementation gate is **12B5 English Writer SQLite v4 repeatability + explicit multi-result retrieval equivalence**. Publish-v4 repeatability has passed exactly at `b921d5350cb14badd9ddf2a65f989ee6eb2c3f03add434e592c674d759c595a9` with 224,478 published / 123,533 default-eligible surfaces. Run `npm run en:db:repeatability`; it performs two full DB builds and verification passes and requires identical semantic fingerprint, counts and database bytes plus zero indexed-vs-full-scan mismatches, including explicit multi-result keys for exact/vowel/family+coda/coda channels. The older 147,904-form DB remains a pre-v4 control snapshot only. Do not start with UI polish, product EN enablement, broad G2P, ranking, or Entity P898 owner work before this DB gate is reviewed.
