...
The current implementation gate is **Phase 12C independent forced-neural g2p-en benchmark**. Source expansion remains accepted at 710,561 / 1,415,550 EN Entity names ready (50.20%), with 704,989 unresolved. Proper-name benchmark v2 remains the accepted control: 600 unique Entity-name tokens with explicit en-US Kaikki/Wiktionary proper-name IPA. MFA is rejected for runtime fallback after full confidence calibration: full exact-tail 71.19%, best 10% by Pynini score 86.67%, only 1.68% retention at >=90% exact-tail and 21.94% at >=85% exact-tail; calibrated fingerprint `9bc00102d46af55352505389c03fe048083cdd81c966007ab86924d9df233488`. Do not promote MFA. The next independent candidate is `g2p-en` 2.1.0 using its bundled Apache-2.0 `checkpoint20.npz`, CMUdict training basis and stressed ARPAbet output. The benchmark must call `G2p.predict()` directly so the package's normal CMUdict/homograph/POS lookup path cannot contaminate the result. DeepPhonemizer is deferred because its pretrained checkpoint license is not separately explicit enough for the intended commercial runtime path. Owner setup/run must use Miniforge Prompt with `(rhymelab-g2pen)` active. No generated pronunciation may be promoted before independent benchmark evidence is reviewed.
