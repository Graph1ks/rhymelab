...
- Bulk Backfill V2 generation additionally requires `source-aware-pronunciation-admission-v1`: only `admit` rows may enter unattended eSpeak generation; `review` and `reject_noise` rows remain preserved and ungenerated. Bulk eSpeak defaults to four batch workers; use the read-only v2 high-speed test to select a stable batch size rather than increasing worker concurrency. Worker/batch tuning may change throughput only, never admission, analyzer, quality-tier, checkpoint, or promotion policy.
