...
- Bulk Backfill V2 generation additionally requires `source-aware-pronunciation-admission-v1`: only `admit` rows may enter unattended eSpeak generation; `review` and `reject_noise` rows remain preserved and ungenerated. Before a full admitted run, use the read-only high-speed ladder on the owner machine and select a stable bounded worker count. Parallelism may change throughput only, never admission, analyzer, quality-tier, checkpoint, or promotion policy.
