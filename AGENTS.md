...
The current implementation gate is **Phase 12B8 read-only English retrieval runtime diagnostic**. Publish-v4 is accepted at `b921d5350cb14badd9ddf2a65f989ee6eb2c3f03add434e592c674d759c595a9`. English DB materialization is accepted at semantic fingerprint `beca46fccb27eed4349c988b726928a464c216b9e59f2640e4925effdc9e6e37`, with 224,478 forms / 123,533 default eligible / 375,321 pronunciations / 173,413 default-profile pronunciations / 181.87 MiB. Full persisted verification now passes all five indexed channels including `multisyllable_key`: 20 general + 20 multi-result samples per channel, zero mismatches, zero foreign-key violations. Run `npm run en:runtime:diagnose` and review `data/local/en-retrieval-runtime-v1-report.json`. The runtime candidate layer must remain read-only, bounded, en-US default-profile only, provenance-preserving, and not wired into the product API/UI. Do not start product EN enablement, final ranking, broad G2P, or Entity P898 owner work before runtime diagnostic/repeatability acceptance.
