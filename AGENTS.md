...
The current implementation gate is **close the multisyllabic English DB retrieval-verifier gap, then begin English retrieval/runtime acceptance**. Publish-v4 is accepted at `b921d5350cb14badd9ddf2a65f989ee6eb2c3f03add434e592c674d759c595a9`. The v4 English DB repeatability gate also passed: semantic fingerprint `beca46fccb27eed4349c988b726928a464c216b9e59f2640e4925effdc9e6e37`, 224,478 forms / 123,533 default eligible / 375,321 pronunciations / 173,413 default-profile pronunciations / 181.87 MiB, with identical fingerprints, counts and bytes across two builds. Review found that `idx_en_pron_multi` existed but the old verifier did not independently require/sample `multisyllable_key`; the source patch now does. After merge run only `npm run en:db:verify` and inspect `data/local/en-writer-db-verification-v1-report.json`; no DB rebuild is required. Do not start UI polish, product EN enablement, broad G2P, ranking, or Entity P898 owner work until that verify passes and retrieval/runtime acceptance begins.
