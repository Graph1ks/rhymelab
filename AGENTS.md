...
Read `docs/ENTITY_SOURCE_ALTERNATIVES_2026-09-18.md` before changing source acquisition. Live 2026-09-18 probes verified QLever selective export as the preferred Phase 12A2 fast-acquisition implementation target: roughly 1.838M all-statement candidates and ~80.8 MB compressed selective Wikidata artifacts before QRank, versus the 103.1 GB classic dump. Match the current JSON importer with `p:/ps:` all-statement P31/P106 semantics, not only `wdt:` truthy relations. Freeze all remote response artifacts locally with exact SPARQL, timestamps, taxonomy hash, row counts, SHA-256 and semantic fingerprint before offline staging. The 20260914 full dump remains retained as optional dated validation/control evidence and is no longer a blocking gate.
