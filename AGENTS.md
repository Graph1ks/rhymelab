...
The current implementation gate is **Phase 12B9 bundled English acceptance evidence**. Publish-v4 and DB storage/indexed retrieval are accepted; DB semantic fingerprint remains `beca46fccb27eed4349c988b726928a464c216b9e59f2640e4925effdc9e6e37`. The owner Phase 12B8 runtime diagnostic passed with semantic fingerprint `dc4de5383325ee3b0d03ca6d77b8282bb0986e19c8e12567c2022a8aa3f29fcf`: all five runtime plans use dedicated indexes, 200/200 stored analyses reanalyze exactly, default-profile leakage is zero, sentinel/variant/derived-inflection checks pass and failed checks are zero. To avoid micro-gates, run `npm run en:acceptance:bundle`; it combines three independent runtime diagnostic DB opens, deterministic usage-stratified query sampling, three Quality/Commonness candidates and four Diversity weights into `data/local/en-acceptance-bundle-v1-report.json`. This bundle is evidence-only and must not enable product EN, broad G2P, final ranking or Entity P898 work.
