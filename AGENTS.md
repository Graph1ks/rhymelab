...
The current implementation gate is **Phase 12C Entity pronunciation source expansion**. The consolidated multilingual evidence owner run is complete with status `evidence_ready` and semantic fingerprint `1861f29aaf0b029d3ac69c1cd6511a0c0b573c47e325efea75e0a9f42b1ec88a`. It preserved the 1,077,644-Entity Hybrid-v2 population and frozen DE Entity runtime fingerprint. English Entity evidence currently resolves 459,728 / 1,415,550 searchable EN names (32.48%); 955,822 remain unresolved. The active owner gate is now `npm run entity:pronunciation:expand`: reuse the existing raw Kaikki `enwiktionary-kaikki-20260916.jsonl.gz`, reuse the full pinned CMUdict, apply conservative Entity-name composition, and optionally record Moby raw evidence. Do not redownload the existing Kaikki/CMUdict artifacts. Do not run mass G2P yet. MFA/DeepPhonemizer/Charsiu exist only behind the proper-name benchmark boundary; Charsiu remains research-only until exact model/data licensing is clean.
