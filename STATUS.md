...
Total Query Pronunciation v1 is implemented for **single-token** unknown user queries with a corrected end-user boundary: source-backed pronunciation remains first priority; only a missing query spelling is resolved to IPA in the **browser/end-user client**. The browser then supplies that ephemeral IPA back to the existing Writer endpoint, where the accepted DE/EN analyzer validates it and the existing Writer/Phrase/Entity retrieval, scoring and ranking paths continue unchanged. The client generator has no Node/`child_process`/host-executable/eSpeak dependency.
