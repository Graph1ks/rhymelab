...
Decision: do **not** freeze `qrank-category-relative-cut-v1`. Keep it as control and evaluate the isolated candidate `category-relative-popularity-hybrid-v1-candidate`, documented in `docs/ENTITY_CUT_HYBRID_V1.md`. The candidate uses a bounded category-relative score (55% QRank percentile, 25% Wikipedia sitelink percentile, 10% DE/EN Wikipedia presence, 7% capped selected external IDs, 3% statement-count percentile) so missing QRank contributes zero to the QRank component without creating a separate binary exclusion gate.

After the candidate PR merges, run only `npm run entity:cut:diagnose:hybrid`. It A/B compares control vs candidate on the existing staged data; no QLever fetch, entity restage, QRank restage or final materialization is required. Do not alter category floors until the ranking policy itself is accepted.
