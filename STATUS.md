...
The deterministic **1000-case unresolved local-data benchmark** is now structurally accepted on the owner's Windows 11 host with eSpeak-NG 1.52.0. The normalized v2 report fingerprint is `facd6d15…ff1e5`: 1012 / 1014 cases are analyzer-compatible (99.80%), all DE strata and product sentinels are 100%, EN preferred Entities are 100%, and EN aliases are 99%. The two failures are non-Latin aliases (`夢境` and `זבולון קוורטין`) for which eSpeak emitted no IPA. 830 / 1012 accepted predictions required adapter normalization, confirming that the v2 normalization layer is material. This is **structural compatibility evidence, not lexical correctness gold**. A deterministic source-backed DE/EN gold-control benchmark is now implemented as the next quality gate; generated staging remains isolated/non-canonical and candidate-overlay/runtime promotion remains blocked pending that evidence.
