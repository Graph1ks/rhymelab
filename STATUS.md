...
Owner-only **Pronunciation Backfill V2** now supersedes the initial V1 collector. A read-only `npm run data:inventory` gate now precedes further local source-path adaptation; it inventories the owner's actual `data/` tree, SQLite schemas and bounded text/JSON structures into `data/local/local-data-inventory-v1-report.json` without assuming historical source locations. V1 was rejected because it started from already accepted/runtime databases and therefore could not see source rows dropped before pronunciation materialization. V2 performs explicit source/stage → accepted diffs: German pre-publish core + raw listed forms vs Writer-v5, original English Kaikki Writer candidates vs accepted analyzed en-US inventory, Phrase catalog/token inventory vs phrase pronunciations, and retained Entity names vs accepted source-backed locale pronunciations. The deduplicated staging workset then runs the existing eSpeak-NG adapter first and sends only analyzer-rejected rows through `client-total-query-pronunciation-v2`. It is batch-checkpointed/resumable, prints progress/rate/ETA, assigns A/B/C/D/U generated-quality tiers, and never auto-promotes canonical data. Owner execution has **not yet been run**. See `docs/PRONUNCIATION_BACKFILL_V2.md`.
