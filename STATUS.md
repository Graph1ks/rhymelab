...
Current immediate gate: Generated Pronunciation Base-Parity V1 owner materialization. DE + EN augmented outputs are resumable and preserved. Phrase full-surface rows are canonical-composition-only: rows blocked solely by intentionally non-active token dependencies are parity-deferred to `data/local/pronunciation-base-parity-v1-deferred-phrases.tsv`; unexplained/active-token gaps remain fatal. Focused continuation: `docs/PRONUNCIATION_BASE_PARITY_HANDOVER.md`. Do not start runtime/UI opt-in integration before `data/local/pronunciation-base-parity-v1-report.json` passes exact DE/EN/Phrase/Entity schema-parity and coverage gates.
