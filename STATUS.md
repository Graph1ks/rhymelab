...
The deterministic **1000-case unresolved local-data benchmark** has now been run once on the owner's Windows 11 host with eSpeak-NG 1.52.0. The sample fingerprint is `c51fad67…34a5`. The first v1 report (`6dde5b58…07c3e`) produced 685 / 1014 analyzer-compatible outputs, but this result is **not accepted as engine-quality evidence** because it exposed an eSpeak-adapter normalization defect: Unicode format joiners split/contaminated diphthongs and affricates, while several eSpeak phone variants were outside the accepted analyzer inventories. The adapter is being normalized without changing the frozen DE/EN analyzers, and a diagnostic v2 rerun is required before staging. Generated bulk pronunciations remain staging-only / opt-in-only; the ~700k unresolved Entity long tail is not being materialized into canonical runtime in this milestone.
