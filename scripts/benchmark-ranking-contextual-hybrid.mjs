...
function metricsForSample(rows, policy, context) {
...
function buildSampleSection(queue, reviews, queryContexts) {
  ...
  const perQuery = [...byQuery.entries()].map(([query, rows]) => {
    ...
    return {
      ...
      policies: Object.fromEntries(POLICIES.map((policy) => [policy, metricsForSample(rows, policy, context)])),
    };
  ...
  return {
    scope: 'fixed reviewed benchmark sample with current local DB query context; candidate lexical provenance can still be absent on legacy queue rows',
    aggregate,
    per_query: perQuery,
  };
}
...
function safetyForRows(rows) {
  ...
  for (const row of rows) {
    const signalRow = row?.usageRank !== undefined || row?.lexiconLayer !== undefined ? row : {
      ...row,
...
const report = {
  schema: 'rhymelab-benchmark-ranking-hybrid-v2',
  ...
  scope: 'second isolated hybrid ranking pass after v1 solved lexical flooding but retained too little Spotify/YouTube gain and still regressed hitzefrei; accepted runtime remains unchanged',
  limitation: 'live comparison reorders only current engine top-250 candidates per query with pool limit 800; any passing policy still requires later retrieval-aware validation',
  policy_notes: {
    baseline_runtime: 'engine_rank remains accepted v0.10 usage-first ordering',
    first_generation_hybrid: 'lexical_hybrid_decade_rare_guard_0_05 retained for direct comparison',
    modern_query_gate: 'new log-commonness policies activate only when current DB query provenance has lexicon_layer=modern; dictionary queries fall back exactly to accepted usage-first ordering',
    ...
    log_commonness: 'ranked candidates use adjusted_score = phonetic_score - lambda * log10(usage_rank), then the existing 0.05 score band; lambdas 0.04/0.05/0.06 are experiment variants, not accepted constants',
    missing_usage: 'ranked evidence sorts before unranked; unranked modern provenance sorts before unranked dictionary; missing usage is never relabeled rare',
    rare_guard: 'explicit source tag rare remains a negative lexical-quality signal',
  },
  policies: POLICIES,
  sample,
  live,
