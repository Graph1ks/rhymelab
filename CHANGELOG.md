...
- Started R9 Shared Core extraction: promoted proven framework-independent search, document, editor, performance, analysis/detail, command and i18n behavior into `packages/shared-core`; isolated IndexedDB/LocalStorage/VisualViewport/browser diagnostics under `packages/platform-web`; replaced the React `src/legacy` bridge with a typed `src/core` boundary; and added a CI guard that forbids React from importing the old `src/studio` or `src/ui` trees.
...
- Completed React Studio owner UX correction round 2: retired the sidebar in favor of a sticky topbar; made Sound Explorer Compact-only with transient filters/details and no horizontal toolbar scroller; removed the Search Tiles renderer; switched editor typography to 50 lazy Google Fonts with Oranienbaum default; added insertion-boundary Bar drag feedback; enlarged Library actions; rebuilt All Rhymes as linked lyric-first rhyme topology; moved shortcut documentation into Commands; reduced Settings to the OKLCH multi-style designer plus Data Safety; and hue-sweep-tested generated Light/Dark contrast.
